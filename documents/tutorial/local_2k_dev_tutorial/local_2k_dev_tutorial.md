...
`Active` 状态变成 `Yes` 之后，你就开始拥有算力了（示例是中 `2KiB` 大小的算力，当前占全网总算力约 `13%`），而本地 `2KiB` 测试网没有最低出块要求（主网 `32GiB` 扇区的最低出块要求是 `10TB` 算力），有算力就可以出块了。因此，有算力之后，后面就是激动人心的开始出块过程了（算力越大，出块越多）。
