...
这里只是预编译，就是为了获取 `./lotus/extern/filecoin-ffi/` 目录下的源码就可以了（不需要编译全部代码），如下图所示（当然你也可以直接从已有的项目中拷贝对应的代码过来）：
...
本文中，我对 **GEF** 做了一些改进，使得我们用起来体验感更好，我自己修改过的 **GEF** 在 [**这里**](./files/.gdbinit-gef.py)，使用的方式是把这个跟文件下载到你的 `home` 目录下，然后在 `home` 目录下执行以下命令使 **GEF** 插件生效（注意这里插件是以 `.` 开头的）：
