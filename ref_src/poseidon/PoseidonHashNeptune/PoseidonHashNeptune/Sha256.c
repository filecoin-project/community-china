...
static const uint32_t g_K[SHA256_BLOCK_SIZE] = 
{
...
void sha256_update(SHA256_CTX *ctx, const uint8_t* data, uint64_t len)
{
	...
	for (i = 0; i < len; ++i) 
    {
		...
		if (ctx->datalen == SHA256_BLOCK_SIZE) 
        {
			...
			ctx->bitlen += (SHA256_DIGEST_LENGTH * 8 * 2);   // 512 bits
			ctx->datalen = 0;
		}
	}
}
...
void sha256_final(SHA256_CTX *ctx, uint8_t dgst[SHA256_DIGEST_LENGTH])
{
	...
	for (i = 0; i < 4; ++i) 
    {
		dgst[i +  0] = (ctx->state[0] >> (24 - i * 8)) & 0x000000ff;
